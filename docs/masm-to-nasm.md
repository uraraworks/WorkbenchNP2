...
`SAKA.ASM` の変換前実測値、全重複ラベル、MZ生成方針、および変換結果と再生成・検証手順は
別文書へ分離してある。ただし題材が1996年の当時ソースそのものであるため、
それらの文書は `samples/legacy/` と同じ扱いで、完成までリポジトリへ含めていない。
ここに書いた変換規則自体は題材に依存しない。
